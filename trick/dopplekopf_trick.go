...
type Dopplekopf[T card.Card] struct {
	cards              [4]T
	openingCard        T
...
func NewTrick[T card.Card](openingPlayerIndex int, cards [4]T) Dopplekopf[T] {
	return Dopplekopf[T]{
		cards:              cards,
		openingCard:        cards[0],
...
func (closed *Dopplekopf[T]) Close() Played {
	return closed
}

func (trick *Dopplekopf[T]) Replay(previousWinnerIndex int) Played {
	...
	for playerIndex, card := range trick.cards {
		...
		//
		// TODO: code goes her: ???card.AssumesParty()???
...
func (closed Dopplekopf[T]) Claims() []Claim {
	return closed.claims[:]
}

func (closed Dopplekopf[T]) Winner() int {
	return closed.currentWinnerIndex
}

func (closed Dopplekopf[T]) Points() uint8 {
